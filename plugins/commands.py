...
async def start(c: Client, m: Message):
    ...
    if len(m.command) == 2:
        ...
        await m.reply_text(
            f"You have successfully connected your API\n\nYour Api: {user_api}\n\nStart sending me posts"
        )
...
async def help_command(c, m: Message):
    ...
    s = (
        HELP_MESSAGE.format(
            firstname=temp.FIRST_NAME, username=temp.BOT_USERNAME, owner="@ask_admin001"
        )
        + cmds
    )
...
async def about_command(c, m: Message):
    ...
    if WELCOME_IMAGE:
        return await m.reply_photo(
            photo=WELCOME_IMAGE,
            caption=ABOUT_TEXT.format(
                bot.mention(style="md"), owner.mention(style="md")
            ),
    ...
    await m.reply_text(
        ABOUT_TEXT.format(bot.mention(style="md"), owner.mention(style="md")),
        disable_web_page_preview=True,
        reply_markup=ABOUT_REPLY_MARKUP,
    )
...
async def shortener_api_handler(bot, m: Message):
    ...
    if len(cmd) == 1:
        s = SHORTENER_API_MESSAGE.format(
            base_site=BASE_SITE, shortener_api=user["shortener_api"]
        )
